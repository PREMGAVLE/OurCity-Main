...
import { Plus, Search, MapPin, Globe, Phone, Edit, Trash2, Eye, X } from 'lucide-react';
...
export const BusinessListings = () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
        {filteredBusinesses.map((business) => (
          <div key={business._id} className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
            <div className="h-32 sm:h-40 md:h-48 bg-gradient-to-r from-purple-600 to-purple-700 relative overflow-hidden">
              <img
            ...
            <div className="p-4 sm:p-5 md:p-6">
              <div className="mb-3 sm:mb-4">
                <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-1 truncate">{business.name}</h3>
                <p className="text-xs sm:text-sm text-purple-600 font-medium">{business.category || 'Uncategorized'}</p>
              </div>
              ...
              <div className="space-y-1.5 sm:space-y-2 mb-3 sm:mb-4">
                ...
                <div className="flex items-center text-xs sm:text-sm text-gray-600">
                  <Globe size={12} className="mr-2 text-gray-400 flex-shrink-0" />
                  <span className="truncate">{business.contact?.website || 'N/A'}</span>
                </div>
              </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
        <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 text-center">
          <div className="text-lg sm:text-2xl font-bold text-purple-600 mb-1 sm:mb-2">{businesses.length}</div>
          <div className="text-xs sm:text-sm text-gray-600">Total Listings</div>
        </div>
        <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 text-center">
          <div className="text-lg sm:text-2xl font-bold text-green-600 mb-1 sm:mb-2">{countByStatus('active')}</div>
          <div className="text-xs sm:text-sm text-gray-600">Active</div>
        </div>
        <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 text-center">
          <div className="text-lg sm:text-2xl font-bold text-yellow-600 mb-1 sm:mb-2">{countByStatus('pending')}</div>
          <div className="text-xs sm:text-sm text-gray-600">Pending</div>
        </div>
        <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 text-center">
          <div className="text-lg sm:text-2xl font-bold text-red-600 mb-1 sm:mb-2">{countByStatus('inactive')}</div>
          <div className="text-xs sm:text-sm text-gray-600">Inactive</div>
        </div>
      </div>
      ...
      {isModalOpen && selectedBusiness && (
        <div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex items-center justify-center">
          <div className="bg-white rounded-xl w-full max-w-2xl p-6 relative">
            <button onClick={() => setIsModalOpen(false)} className="absolute top-3 right-3 text-gray-600 hover:text-black">
              <X size={18} />
            </button>
            <h2 className="text-lg font-bold mb-4">Edit Business</h2>

            <div className="space-y-4 max-h-[70vh] overflow-y-auto">
              <input name="name" value={selectedBusiness.name} onChange={handleFormChange} placeholder="Name" className="w-full border p-2 rounded" />
              <textarea name="description" value={selectedBusiness.description} onChange={handleFormChange} placeholder="Description" className="w-full border p-2 rounded" />
              <input name="category" value={selectedBusiness.category} onChange={handleFormChange} placeholder="Category" className="w-full border p-2 rounded" />
              <input name="address.street" value={selectedBusiness.address.street} onChange={handleFormChange} placeholder="Street" className="w-full border p-2 rounded" />
              <input name="address.city" value={selectedBusiness.address.city} onChange={handleFormChange} placeholder="City" className="w-full border p-2 rounded" />
              <input name="address.state" value={selectedBusiness.address.state} onChange={handleFormChange} placeholder="State" className="w-full border p-2 rounded" />
              <input name="address.pincode" value={selectedBusiness.address.pincode} onChange={handleFormChange} placeholder="Pincode" className="w-full border p-2 rounded" />
              <input name="contact.phone" value={selectedBusiness.contact.phone} onChange={handleFormChange} placeholder="Phone" className="w-full border p-2 rounded" />
              <input name="contact.email" value={selectedBusiness.contact.email} onChange={handleFormChange} placeholder="Email" className="w-full border p-2 rounded" />
              <input name="contact.website" value={selectedBusiness.contact.website} onChange={handleFormChange} placeholder="Website" className="w-full border p-2 rounded" />
            </div>

            <div className="flex justify-end mt-5">
              <button onClick={handleUpdate} className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-lg">Update</button>
            </div>
          </div>
        </div>
      )}
